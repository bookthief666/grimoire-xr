...
//
// Source: bookthief666/liber-333-grimoire, src/data/liber333.js
// Ported verbatim: chapter order, source text, editorial commentary and
// correspondences are byte-identical to the original edition.
//
// The corpus holds 94 records: two preliminary veil entries (chapters -2 and
// -1, '?' and '!') followed by Chapters 0 through 91. It is NOT 'chapters
// 0-93' — that description was wrong in this repository before this port, and
// produced draws for two chapters that do not exist while making the veils
// unreachable. Selection works on ARRAY INDEX; read the record's own
// `chapter` field for its identity.
//
// Provenance is per field, not per record. See LIBER333_FIELD_PROVENANCE in
// ../provenance.ts: `text`/`title` are Crowley's published source text,
// `commentary` is modern editorial interpretation supplied by this edition,
// and the four correspondence fields are supplied by the edition rather than
// attested in the source work.

export type ChapterRecord = {
  /** The edition's own chapter number. -2 and -1 are the two veils. */
  chapter: number
  /** Source text — Crowley. */
  title: string
  /** Source text — Crowley. */
  text: string
  /** Modern editorial commentary — this edition, NOT Crowley. */
  commentary: string
  /** Correspondence supplied by the edition. */
  sephira: string
  /** Correspondence supplied by the edition. '—' on the veils. */
  path: string
  /** Correspondence supplied by the edition. */
  element: string
  /** Correspondence supplied by the edition. '—' where none is assigned. */
  tarot: string
}

export const LIBER_333: readonly ChapterRecord[] = [
  {
    "chapter": -2,
    "title": "?",
    "text": "?",
